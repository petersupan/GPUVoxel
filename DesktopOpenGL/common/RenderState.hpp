...
struct LandscapeImRenderState : public SimpleRenderState
{
	...
	{
		SimpleRenderState::setParameters(e);
		LandscapeImShaderEffect* effect = static_cast<LandscapeImShaderEffect*>(e);
		check_gl_error();
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, heightTexId);
		glUniform1i(effect->heightSamplerId, 1);
		check_gl_error();
		glUniform3fv(effect->cameraPositionId, 1, &cameraPosition[0]);
	}
};


struct LandscapeGenerationRenderState : public SimpleRenderState
{
	unsigned int outImageId;

	virtual void setParameters(ShaderEffect* e)
	{
		SimpleRenderState::setParameters(e);
		LandscapeGenerationShaderEffect* effect = static_cast<LandscapeGenerationShaderEffect*>(e);
		check_gl_error();
		//glActiveTexture(GL_TEXTURE1);
		//glBindTexture(GL_TEXTURE_2D, outImageId);
		glBindImageTexture(0, outImageId, 0, false, 0, GL_WRITE_ONLY, GL_RGBA8);
		check_gl_error();
		glUniform1i(effect->imageId, 0);
		check_gl_error();
	}
};
